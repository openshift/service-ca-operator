...
func (sc *serviceServingCertController) issuedByCurrentCA(secret *corev1.Secret) bool {
	...
	if certs[0].Issuer.CommonName != sc.commonName() {
		return false
	}
	...
	checkKeyIdentifierChaining := len(caSubjectKeyId) > 0
	if checkKeyIdentifierChaining {
...
func GetServingCert(dnsSuffix string, ca *crypto.CA, intermediateCACert *x509.Certificate, objMeta *metav1.ObjectMeta) (*crypto.TLSCertificateConfig, error) {
	dnsName := objMeta.Name + "." + objMeta.Namespace + ".svc"
	...
	servingCert, err := ca.MakeServerCert(
		...
		certificateLifetime,
		cryptoextensions.ServiceServerCertificateExtensionV1(objMeta.UID),
	)
	if err != nil {
		return nil, err
	}
...
func toRequiredSecret(dnsSuffix string, ca *crypto.CA, intermediateCACert *x509.Certificate, service *corev1.Service, secretCopy *corev1.Secret) error {
	servingCert, err := GetServingCert(dnsSuffix, ca, intermediateCACert, &service.ObjectMeta)
	if err != nil {
		return err
	}
