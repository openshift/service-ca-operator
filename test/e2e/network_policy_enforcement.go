...
func testMetricsOpenButOtherPortsBlocked() {
	...
	g.GinkgoWriter.Printf("expecting deny from openshift-etcd with custom label to %s:8443 (custom app labels not allowed in etcd namespace)\n", svcCAOperatorIP)
	expectConnectivity(kubeClient, "openshift-etcd", map[string]string{"test": "client"}, svcCAOperatorIP, 8443, false)
...
func testMetricsIngressOpenAccess() {
	...
	g.GinkgoWriter.Printf("expecting deny from openshift-etcd with custom label to %s:8443 (custom app labels not allowed in etcd namespace)\n", svcCAOperatorIP)
	expectConnectivity(kubeClient, "openshift-etcd", map[string]string{"test": "metrics-client"}, svcCAOperatorIP, 8443, false)

	g.By("Testing metrics policy: console namespace with custom app label can access metrics")
	g.GinkgoWriter.Printf("expecting allow from openshift-console with custom label to %s:8443 (metrics open to most namespaces)\n", svcCAOperatorIP)
	expectConnectivity(kubeClient, "openshift-console", map[string]string{"custom-app": "test-client"}, svcCAOperatorIP, 8443, true)
